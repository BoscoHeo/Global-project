        ...
        <main className="flex-1 flex flex-col p-6 md:p-8 lg:overflow-y-auto">
                  ...
                  {(teacherGroupPasscodeList.length > 0 ? teacherGroupPasscodeList : [
                    ...
                    <div key={grp.groupName} className="bg-white rounded-2xl border-2 border-indigo-150 p-4 shadow-xs flex flex-col justify-between hover:border-indigo-300 hover:shadow-md transition">
                      <div>
                        ...
                        <div className="my-3 bg-white rounded-xl p-3 text-center border-2 border-indigo-100 shadow-inner">
                          <span className="text-[11px] text-slate-500 font-bold block mb-1 whitespace-nowrap">현재 적용된 모둠 비밀번호</span>
                          {grp.isSet ? (
                            <div className="inline-flex items-center justify-center gap-2 bg-indigo-50 border border-indigo-200 px-4 py-1.5 rounded-xl">
                              <Lock className="w-4 h-4 text-indigo-600 shrink-0" />
                              <span className="text-xl font-black font-mono tracking-widest text-indigo-700 whitespace-nowrap">
                                {grp.passcode}
                              </span>
                            </div>
                          ) : (
                            <span className="text-xs font-bold text-slate-400 bg-slate-100 px-3 py-1 rounded-lg inline-block whitespace-nowrap">
                              미등록 (자유 입장)
                            </span>
                          )}
                        </div>
                      ...
                      <div className="mt-3 pt-3 border-t border-slate-150 space-y-2">
                        <label className="text-[10.5px] font-extrabold text-slate-500 uppercase tracking-wider block whitespace-nowrap">
                          선생님 지정 새 비밀번호
                        </label>
                        <div className="flex gap-2">
                          <input
                            ...
                            className="w-full bg-white border-2 border-slate-200 focus:border-indigo-500 rounded-xl px-3 py-2 text-center text-sm font-black tracking-wider text-slate-900 focus:outline-none transition shadow-2xs"
                          />
                          <button
                            onClick={() => handleTeacherSetGroupPasscode(grp.groupName)}
                            className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-black transition shrink-0 cursor-pointer shadow-xs whitespace-nowrap"
                          >
                            저장
                          </button>
                        </div>
                        ...
                        {grp.isSet && (
                          <button
                            ...
                            className="w-full py-1.5 text-rose-600 hover:text-rose-700 hover:bg-rose-50 rounded-lg text-[10.5px] font-bold transition flex items-center justify-center gap-1 cursor-pointer whitespace-nowrap"
                          >
                            <span>비밀번호 초기화 (미등록으로)</span>
                          </button>
                        )}
                      </div>
                    </div>
          ...
          {activeTab === "teacher" && (
            isTeacherUnlocked ? (
              ...
              {/* 🔐 우리 반 4개 모둠 비밀번호(PIN) 교사 설정 센터 (교사 허브 상단 배치) */}
              <div className="w-full bg-gradient-to-r from-indigo-50/80 via-purple-50/60 to-pink-50/40 border-2 border-indigo-200/90 rounded-3xl p-6 shadow-sm space-y-4">
                <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-indigo-150/70">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-indigo-600 text-white flex items-center justify-center shadow-md shadow-indigo-200">
                      <Lock className="w-5 h-5" />
                    </div>
              ...
              <div className="bg-white border-2 border-amber-300 rounded-3xl p-6 shadow-sm space-y-5">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-amber-100">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-2xl bg-amber-500 text-white flex items-center justify-center shadow-md shadow-amber-100">
                      <ShoppingCart className="w-5 h-5" />
                    </div>
                    <div>
                      <h3 className="text-base font-black text-slate-900 flex items-center gap-2">
                        <span>{classCode || "6-1"} 부스 준비물 & 반별 35만원 예산 일괄 수합 센터</span>
                        <span className="text-[10px] bg-amber-100 text-amber-800 font-extrabold px-2.5 py-0.5 rounded-full border border-amber-200">
                          실시간 자동 집계
                        </span>
                      </h3>
                      <p className="text-xs text-slate-500 font-medium mt-0.5">
                        아이들이 입력한 준비물 데이터를 한 번에 싹 받아오고, 버튼 클릭 한 번으로 구글 스프레드시트에 복사(Ctrl+V)하거나 엑셀로 내려받습니다.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0 flex-wrap">
                    <button
                      onClick={() => fetchMaterialsSummary()}
                      disabled={loadingMaterialsSummary}
                      className="px-3.5 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl shadow-xs transition flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                    >
                      <RefreshCw className={`w-3.5 h-3.5 ${loadingMaterialsSummary ? "animate-spin" : ""}`} />
                      <span>{loadingMaterialsSummary ? "수합 중..." : "실시간 최신 수합"}</span>
                    </button>
                    ...
                    <button
                      ...
                      title="모든 모둠의 준비물을 구글 시트에 바로 붙여넣을 수 있게 복사합니다."
                    >
                      <Copy className="w-3.5 h-3.5" />
                      <span>전 모둠 시트 복사 (Ctrl+V)</span>
                    </button>
                ...
                {/* Summary Budget & Group Breakdown */}
                {(() => {
                  let totalItemsCount = 0;
                  let totalAmount = 0;
                  const groupBudgets: { [key: string]: { count: number; sum: number; intro: string; country: string } } = {};
                  ...
                  return (
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 bg-amber-50/60 p-4 rounded-2xl border border-amber-200">
                        <div>
                          <div className="text-[10px] font-bold text-slate-400">학급 전체 신청 총액</div>
                          <div className="text-lg font-black text-indigo-700 mt-0.5">{totalAmount.toLocaleString()}원</div>
                          <div className="text-[10px] text-slate-500 mt-0.5">총 등록 품목: {totalItemsCount}건</div>
                        </div>
                        <div>
                          <div className="text-[10px] font-bold text-slate-400">잔여 가능 예산 (35만원 한도)</div>
                          <div className={`text-lg font-black mt-0.5 ${remaining < 0 ? "text-rose-600" : "text-emerald-600"}`}>
                            {remaining < 0 ? `-${Math.abs(remaining).toLocaleString()}원 (초과)` : `${remaining.toLocaleString()}원 남음`}
                          </div>
                          <div className="text-[10px] text-slate-500 mt-0.5">예산 소진율: {percent}%</div>
                        </div>
                        <div>
                          <div className="text-[10px] font-bold text-slate-400">예산 집행 게이지</div>
                          <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden mt-2">
                            <div 
                              className={`h-full ${remaining < 0 ? "bg-rose-500" : percent > 85 ? "bg-amber-500" : "bg-emerald-500"}`} 
                              style={{ width: `${percent}%` }}
                            />
                          </div>
                      ...
                      <div className="overflow-x-auto rounded-xl border border-slate-200">
                        <table className="w-full text-left text-xs border-collapse">
                          ...
                          <tbody className="divide-y divide-slate-100">
                            {summaryClassMaterials.flatMap((grp: any) => 
                              (grp.materials || []).map((item: BoothMaterialItem, idx: number) => (
                                <tr key={`tch_${grp.groupName}_${item.id}_${idx}`} className="hover:bg-slate-50 transition">
                                  <td className="py-2.5 px-3 font-extrabold text-indigo-700 whitespace-nowrap">
                                    {grp.groupName}
                                  </td>
                                  <td className="py-2.5 px-3 whitespace-nowrap">
                                    <span className={`text-[10px] font-black px-2 py-0.5 rounded-full border ${
                                      item.market === "아이스크림몰" ? "bg-amber-100 text-amber-900 border-amber-200" :
                                      item.market === "11번가" ? "bg-rose-100 text-rose-900 border-rose-200" :
                                      item.market === "지마켓" ? "bg-emerald-100 text-emerald-900 border-emerald-200" :
                                      "bg-slate-100 text-slate-700 border-slate-200"
                                    }`}>
                                      {item.market}
                                    </span>
                                  ...
                                  <td className="py-2.5 px-3 text-[11px] text-slate-500">{item.note || "-"}</td>
                                  <td className="py-2.5 px-3 text-center whitespace-nowrap">
                                    <button
                                      type="button"
                                      onClick={() => copyItemToClipboard(item)}
                                      className="px-2 py-1 bg-amber-50 hover:bg-amber-100 text-amber-900 border border-amber-200 rounded-lg text-[10.5px] font-bold transition flex items-center gap-0.5 mx-auto cursor-pointer"
                                    >
                                      <Copy className="w-2.5 h-2.5" />
                                      <span>시트 복사</span>
                                    </button>
                            ...
                            {summaryClassMaterials.every(g => !g.materials || g.materials.length === 0) && (
                              <tr>
                                <td colSpan={10} className="py-8 text-center text-slate-400 text-xs">
                                  현재 수합된 준비물 품목이 없습니다. 아이들이 등록한 후 [실시간 최신 수합]을 눌러보세요.
                                </td>
                              </tr>
                            )}
                          </tbody>
